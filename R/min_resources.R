                          ...
                          conf = 0.95)
{
  ...
  if(!is.null(resources) && !all(resources %in% allowed_resources)){
  ...
  if ('M' %in% resources && model@protocol != 'PCR_rep'){
    if (model@protocol == 'seq_read'){
      ...
      for (n in 1:N){
        for (i in 1:I){
          for (c in 1:C){
            ...
            for(j in 1:J){
              # Rebuild A array conditionally on Z = 1 (i.e. species present at site)
              A[, n, i, j, c] <- rbinom(n = sims, size = 1, prob = Z[, n, i, c] * plogis(posterior_theta[, n, i, j, c]))
              # Rebuild S array (S >= 1 when species is present and collected)
              S_raw <- rnbinom(n = sims, size = delta, prob = delta / (delta + exp(posterior_phi[, n, i, j, c])))
              S[, n, i, j, c] <- pmax(S_raw, 1)
            }
          }
        }
      }
      ...
      for (i in 1:I){
        for (j in 1:J){
          for (c in 1:C){
            # Compute denominator across all species at once
            denom <- apply(A[, , i, j, c] * S[, , i, j, c], 1, sum, na.rm = TRUE)
            denom <- ifelse(denom > 0, denom, 1)
            
            for (n in 1:N){
              posterior_pi[, n, i, j, c] <- A[, n, i, j, c] * S[, n, i, j, c] / denom
              
              # Condition on species being present and collected (A = 1)
              post_pi <- posterior_pi[A[, n, i, j, c] == 1, n, i, j, c]
              
              # Fallback if no samples with A = 1
              if (length(post_pi) == 0){
                M_min$median[n, i, j, c] <- NA
                M_min$hdi1[n, i, j, c]   <- NA
                M_min$hdi2[n, i, j, c]   <- NA
                }else{
        ...
        for (i in 1:I){
          for (c in 1:C){
            ...
            for(j in 1:J){
              ...
              A[, n, i, j, c] <- rbinom(n = sims, size = 1, prob = Z[, n, i, c] * plogis(posterior_theta[, n, i, j, c]))
              for (k in 1:K){
                # Rebuild W array conditionally on A = 1 (i.e. species detected in a sample)
                W[, n, i, j, k, c] <- rbinom(n = sims, size = 1,prob = A[, n, i, j, c] * plogis(posterior_p[, n, i, j, k, c]))
                # Rebuild S array (S >= 1 when species is present and collected)
                S_raw <- rnbinom(n = sims, size = delta, prob = delta / (delta + exp(posterior_phi[, n, i, j, k, c])))
                S[, n, i, j, k, c] <- pmax(S_raw, 1)
            }
          }
        }
      }
      ...
      for (i in 1:I){
        for (j in 1:J){
          for (k in 1:K){
            for(c in 1:C){
              # Compute denominator across all species at once
              denom <- apply(W[, , i, j, k, c] * S[, , i, j, k, c], 1, sum, na.rm = TRUE)
              denom <- ifelse(denom > 0, denom, 1)
            
              for (n in 1:N){
                posterior_pi[, n, i, j, k, c] <- W[, n, i, j, k, c] * S[, n, i, j, k, c] / denom
              
                # Condition on species being present and collected (W = 1)
                post_pi <- posterior_pi[W[, n, i, j, k, c] == 1, n, i, j, k, c]
              
                # Fallback if no samples with A = 1
                if (length(post_pi) == 0){
                  M_min$median[n, i, j, k, c] <- NA
                  M_min$hdi1[n, i, j, k, c]   <- NA
                  M_min$hdi2[n, i, j, k, c]   <- NA
                }else{
