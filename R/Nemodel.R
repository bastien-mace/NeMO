...
#' @param delta Numeric. Dispersion parameter of the negative binomial distribution \eqn{S} (only for `'seq_read'` or `'PCR_rep_seq_read'` protocols, default: 1).
...
#'   - `delta`: Numeric. Dispersion parameter used to model the negative binomial distribution \eqn{S}.
#'
                    ...
                    delta = 1, ...)
{
  ...
  if (delta <= 0){
    stop("Error: 'delta' should be positive.")
  }
  ...
  if (protocol == 'PCR_rep'){
    ...
    # Set the dimensions for parameter p
    p_dims <- list(i = FALSE, j = FALSE, k = FALSE, c = FALSE)
    if (!is.null(p_cov)){
      p_cov_dims <- dim(p_cov)  # [I, J, K, C, nb_p_cov]
      p_dims$i <- (p_cov_dims[1] > 1)
      p_dims$j <- (p_cov_dims[2] > 1)
      p_dims$k <- (p_cov_dims[3] > 1)
      p_dims$c <- (p_cov_dims[4] > 1)
    }
    
    # Set the corresponding indices for parameter p
    p_indices <- 'n'
    if (p_dims$i) p_indices <- paste0(p_indices, ', i')
    if (p_dims$j) p_indices <- paste0(p_indices, ', j')
    if (p_dims$k) p_indices <- paste0(p_indices, ', k')
    if (p_dims$c) p_indices <- paste0(p_indices, ', c')
    p_indices <- paste0('[', p_indices, ']')
    
    # Set the corresponding indices for p_cov
    p_cov_indices <- ''
    if (!is.null(p_cov)){
      p_cov_indices <- ifelse(p_dims$i, 'i, ', '1, ')
      p_cov_indices <- paste0(p_cov_indices, ifelse(p_dims$j, 'j, ', '1, '))
      p_cov_indices <- paste0(p_cov_indices, ifelse(p_dims$k, 'k, ', '1, '))
      p_cov_indices <- paste0(p_cov_indices, ifelse(p_dims$c, 'c, ', '1, '))
    }
    
    # Calculate p outside useless loops if necessary
    model <- "
model
{
  # Replicate-level presence/absence (= Detection)
  for (n in 1:N){"
    if (!p_dims$i && !p_dims$j && !p_dims$k && !p_dims$c){
      model <- paste(model, "
    logit(p[n]) <- alpha_p[n]")
    ...
    if (p_dims$i && !p_dims$j && !p_dims$k && !p_dims$c){
      ...
      if (!is.null(p_cov)){
        for(q in 1:nb_p_cov){
          model <- paste(model, paste0("
                                   + beta_p[n, ", q, "] * p_cov[", p_cov_indices, q, "]"))
        }
      }
    }
    ...
    model <- paste0(model, "
      for (j in 1:J){")
    if (p_dims$j && !p_dims$k && !p_dims$c){
      model <- paste(model, paste0("
        logit(p", p_indices, ") <- alpha_p[n]"))
    ...
    model <- paste0(model, "
            Y[n, i, j, k, c] ~ dbern(A[n, i, j, c] * p", p_indices, ")")
    if(loglik){
      model <- paste(model, paste0("
            loglik[n, i, j, k, c] <- logdensity.bern(Y[n, i, j, k, c], A[n, i, j, c] * p", p_indices, ")"))
    }
  ...
  if (protocol == 'seq_read'){
    ...
    # Set the dimensions for parameter phi
    phi_dims <- list(i = FALSE, j = FALSE, c = FALSE)
    if (!is.null(phi_cov)){
      phi_cov_dims <- dim(phi_cov)  # [I, J, C, nb_phi_cov]
      phi_dims$i <- (phi_cov_dims[1] > 1)
      phi_dims$j <- (phi_cov_dims[2] > 1)
      phi_dims$c <- (phi_cov_dims[3] > 1)
    }
      
    # Set the corresponding indices for parameter phi
    phi_indices <- 'n'
    if (phi_dims$i) phi_indices <- paste0(phi_indices, ', i')
    if (phi_dims$j) phi_indices <- paste0(phi_indices, ', j')
    if (phi_dims$c) phi_indices <- paste0(phi_indices, ', c')
    phi_indices <- paste0('[', phi_indices, ']')
      
    # Set the corresponding indices for phi_cov
    phi_cov_indices <- ''
    if (!is.null(phi_cov)){
      phi_cov_indices <- ifelse(phi_dims$i, 'i, ', '1, ')
      phi_cov_indices <- paste0(phi_cov_indices, ifelse(phi_dims$j, 'j, ', '1, '))
      phi_cov_indices <- paste0(phi_cov_indices, ifelse(phi_dims$c, 'c, ', '1, '))
    }
      
    # Calculate phi here if varying among i, j, c
    # (we should calculate phi outside the loops if varying only on n)
    if (phi_dims$i || phi_dims$j || phi_dims$c){
      model <- "
model
{
  ...
  for (i in 1:I){
    for (j in 1:J){
      ...
      model <- paste(model, paste0("
          log(phi", phi_indices, ") <- alpha_phi[n]"))
      ...
      if (!is.null(phi_cov)){
        for(q in 1:nb_phi_cov){
          model <- paste(model, paste0("
                                                    + beta_phi[n, ", q, "] * phi_cov[", phi_cov_indices, q, "]"))
        }
      }
      ...
      model <- paste(model, "
          pi[n, i, j, c] <- A[n, i, j, c] * S[n, i, j, c] / ifelse(sum(A[1:N, i, j, c] * S[1:N, i, j, c]) > 0,
                                                                   ...
                                                                   1)
          S[n, i, j, c] ~ dnegbin(delta / (delta + phi", phi_indices, "),
                                  delta)
        }
      }
    }
    ...
    # Calculate phi here if varying only on n
    if (!phi_dims$i && !phi_dims$j && !phi_dims$c){
      model <- "
model
{
  for (n in 1:N){
    log(phi[n]) <- alpha_phi[n]"
  
      if (!is.null(phi_cov_sp)){
        for(q in 1:nb_phi_cov_sp){
          model_header <- paste(model_header, paste0("
                                + beta_phi_sp[", q, "] * phi_cov_sp[n, ", q, "]"))
      }
    }
    ...
    model <- paste(model, "
        for (n in 1:N){
          pi[n, i, j, c] <- A[n, i, j, c] * S[n, i, j, c] / ifelse(sum(A[1:N, i, j, c] * S[1:N, i, j, c]) > 0,
                                                                   sum(A[1:N, i, j, c] * S[1:N, i, j, c]),
                                                                   1)
          S[n, i, j, c] ~ dnegbin(delta / (delta + phi[n]),
                                  delta)
        }
      }
    }
  ...
  if (protocol == 'PCR_rep_seq_read'){
    ...
    # Set the dimensions for parameter phi
    phi_dims <- list(i = FALSE, j = FALSE, k = FALSE, c = FALSE)
    if (!is.null(phi_cov)){
      phi_cov_dims <- dim(phi_cov)  # [I, J, K, C, nb_phi_cov]
      phi_dims$i <- (phi_cov_dims[1] > 1)
      phi_dims$j <- (phi_cov_dims[2] > 1)
      phi_dims$k <- (phi_cov_dims[3] > 1)
      phi_dims$c <- (phi_cov_dims[4] > 1)
    }
    ...
    if (phi_dims$i) phi_indices <- paste0(phi_indices, ', i')
    if (phi_dims$j) phi_indices <- paste0(phi_indices, ', j')
    if (phi_dims$k) phi_indices <- paste0(phi_indices, ', k')
    if (phi_dims$c) phi_indices <- paste0(phi_indices, ', c')
    phi_indices <- paste0('[', phi_indices, ']')
    ...
    if (!is.null(phi_cov)){
      phi_cov_indices <- ifelse(phi_dims$i, 'i, ', '1, ')
      phi_cov_indices <- paste0(phi_cov_indices, ifelse(phi_dims$j, 'j, ', '1, '))
      phi_cov_indices <- paste0(phi_cov_indices, ifelse(phi_dims$k, 'k, ', '1, '))
      phi_cov_indices <- paste0(phi_cov_indices, ifelse(phi_dims$c, 'c, ', '1, '))
    }
    
    # Calculate phi here if varying among i, j, c
    # (we should calculate phi outside the loops if varying only on n)
    if (phi_dims$i || phi_dims$j || phi_dims$k || phi_dims$c){
      model <- "
model
{
  # Replicate-level sequence read count
  ...
  model <- paste(model, "
            pi[n, i, j, k, c] <- W[n, i, j, k, c] * S[n, i, j, k, c] / ifelse(sum(W[1:N, i, j, k, c] * S[1:N, i, j, k, c]) > 0,
                                                                              ...
                                                                              1)
            S[n, i, j, k, c] ~ dnegbin(delta / (delta + phi", phi_indices, "),
                                       delta)
          }
        }
      }
...
model
{
      ...
      model <- paste(model, "
          for (n in 1:N){
            pi[n, i, j, k, c] <- W[n, i, j, k, c] * S[n, i, j, k, c] / ifelse(sum(W[1:N, i, j, k, c] * S[1:N, i, j, k, c]) > 0,
                                                                              sum(W[1:N, i, j, k, c] * S[1:N, i, j, k, c]),
                                                                              1)
            S[n, i, j, k, c] ~ dnegbin(delta / (delta + phi[n]),
                                       delta)
          }
        }
      }
  ...
  model <- paste0(model, "
            W[n, i, j, k, c] ~ dbern(A[n, i, j, c] * p", p_indices, ")")
  ...
  # Set the dimensions for parameter theta     
  theta_dims <- list(i = FALSE, j = FALSE, c = FALSE)
  if (!is.null(theta_cov)){
    theta_cov_dims <- dim(theta_cov)  # [I, J, C, nb_theta_cov]
    theta_dims$i <- (theta_cov_dims[1] > 1)
    theta_dims$j <- (theta_cov_dims[2] > 1)
    theta_dims$c <- (theta_cov_dims[3] > 1)
  }
        
  # Set the corresponding indices for parameter theta
  theta_indices <- 'n'
  if (theta_dims$i) theta_indices <- paste0(theta_indices, ', i')
  if (theta_dims$j) theta_indices <- paste0(theta_indices, ', j')
  if (theta_dims$c) theta_indices <- paste0(theta_indices, ', c')
  theta_indices <- paste0('[', theta_indices, ']')
        
  # Set the corresponding indices for theta_cov
  theta_cov_indices <- ''
  if (!is.null(theta_cov)){
    theta_cov_indices <- ifelse(theta_dims$i, 'i, ', '1, ')
    theta_cov_indices <- paste0(theta_cov_indices, ifelse(theta_dims$j, 'j, ', '1, '))
    theta_cov_indices <- paste0(theta_cov_indices, ifelse(theta_dims$c, 'c, ', '1, '))
  }
        
  # Calculate theta outside useless loops if necessary
  model <- paste(model, "
  # Sample-level presence/absence (= Collection)
  for (n in 1:N){")
  if (!theta_dims$i && !theta_dims$j && !theta_dims$c){
    model <- paste(model, "
    logit(theta[n]) <- alpha_theta[n]")
    if (!is.null(theta_cov_sp)){
  ...
  if (theta_dims$i && !theta_dims$j && !theta_dims$c){
    ...
    if (!is.null(theta_cov)){
      for(q in 1:nb_theta_cov){
        model <- paste(model, paste0("
                                           + beta_theta[n, ", q, "] * theta_cov[", theta_cov_indices, q, "]"))
      }
    }
  }
  ...
  model <- paste0(model, "
      for (j in 1:J){")
  if (theta_dims$j && !theta_dims$c){
    model <- paste(model, paste0("
        logit(theta", theta_indices, ") <- alpha_theta[n]"))
    if (!is.null(theta_cov_sp)){
      for(q in 1:nb_theta_cov_sp){
        model <- paste(model, paste0("
                                                          + beta_theta_sp[", q, "] * theta_cov_sp[n, ", q, "]"))
  ...
  model <- paste0(model, "
          A[n, i, j, c] ~ dbern(Z[n, i, c] * theta", theta_indices, ")
        }
      }
    }
  }")
  
  # Set the dimensions for parameter psi
  psi_dims <- list(i = FALSE, c = FALSE)
  if (!is.null(psi_cov)){
    psi_cov_dims <- dim(psi_cov)  # [I, C, nb_psi_cov]
    psi_dims$i <- (psi_cov_dims[1] > 1)
    psi_dims$c <- (psi_cov_dims[2] > 1)
  }
        
  # Set the corresponding indices for parameter psi
  psi_indices <- 'n'
  if (psi_dims$i) psi_indices <- paste0(psi_indices, ', i')
  if (psi_dims$c) psi_indices <- paste0(psi_indices, ', c')
  psi_indices <- paste0('[', psi_indices, ']')
        
  # Set the corresponding indices for psi_cov
  psi_cov_indices <- ''
  if (!is.null(psi_cov)){
    psi_cov_indices <- ifelse(psi_dims$i, 'i, ', '1, ')
    psi_cov_indices <- paste0(psi_cov_indices, ifelse(psi_dims$c, 'c, ', '1, '))
  }
  ...
  for (n in 1:N){")
  if (!psi_dims$i && !psi_dims$c){
    model <- paste(model, "
    logit(psi[n]) <- alpha_psi[n]")
    if (!is.null(psi_cov_sp)){
  ...
  if (psi_dims$i && !psi_dims$c){
    ...
    if (!is.null(psi_cov)){
      for(q in 1:nb_psi_cov){
        model <- paste(model, paste0("
                                       + beta_psi[n, ", q, "] * psi_cov[", psi_cov_indices, q, "]"))
      }
    }
  }
  ...
  if (psi_dims$c){
    model <- paste(model, paste0("
        logit(psi", psi_indices, ") <- alpha_psi[n]"))
    if (!is.null(psi_cov_sp)){
      for(q in 1:nb_psi_cov_sp){
        model <- paste(model, paste0("
                                                    + beta_psi_sp[", q, "] * psi_cov_sp[n, ", q, "]"))
  ...
  for (c in 1:nb_init_lists){
    ...
    if (protocol == 'PCR_rep_seq_read'){
      init_chain <- c(init_chain,
                      ...
                      list(S = Y + 1))  # We add 1 to avoid null values
    }
    if (protocol == 'seq_read'){
      init_chain <- c(init_chain,
                      list(S = Y + 1)) # We add 1 to avoid null values
